...
DATABASE = os.environ.get("MYSQL_DATABASE") or os.environ.get("DB_NAME", "excel_cleaner_db")
...
try:
    ...
    # Drop tables cleanly (MySQL foreign key checks disabled)
    try:
        cur.execute("SET FOREIGN_KEY_CHECKS = 0")
        tables_to_drop = [
            'master_records', 'rejected_records', 'uploaded_files', 'field_registry',
            'field_aliases', 'rule_presets', 'api_tokens', 'logs', 'login_attempts',
            'cleaning_jobs', 'search_logs', 'user_change_requests', 'user_notifications',
            'role_export_limits', 'user_daily_exports', 'client_api_keys', 'master_columns_registry', 'users'
        ]
